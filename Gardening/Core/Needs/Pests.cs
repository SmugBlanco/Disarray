...
namespace Disarray.Gardening.Core.Needs
{
	public class Pests : PlantNeeds
	{
		...
		public sealed override string DisplayIcon => "Disarray/Gardening/Core/Needs/Pests";
		...
		public override void Load(TagCompound tagCompound)
		{
			...
			IEnumerable<PestEntity> savedPests = savedPestsNames.Select(pest => PestEntity.CreateNewInstance(GetClass<PestEntity>().GetData<PestEntity>(pest), SourcePlant));
			CurrentPests = new HashSet<PestEntity>(savedPests);
		}
	}
}